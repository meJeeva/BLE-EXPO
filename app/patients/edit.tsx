...
export default function EditPatient() {
  ...
  const { bootstrapData, fetchBootstrap } = useAuthStore();
  ...
  useEffect(() => {
    const patient = bootstrapData?.patients?.find((p: any) => p.id === params.id);
    if (patient) {
      setFormData({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await api.updatePatient(params.id, payload);
